...
public class MonticuloArreglo<T extends ComparableIndexable<T>>
    ...
    public MonticuloArreglo(Iterable<T> iterable, int n) {
        ...
        for(T e : iterable)
            agregaEn(e, elementos++);
    }

    private void agregaEn(T elemento,int indice){
    ...
    @Override public T elimina() {
        if(elementos==0)
            throw new IllegalStateException("El monticulo esta vacío.");
        ...
        for (T elemento : arreglo)
              if (elemento != null && elemento.compareTo(minimo) <= 0)
                minimo = elemento;
